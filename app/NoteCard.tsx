...
export default function NoteCard({ note, links = [] }: Props) {
  ...
  const saveTags = async (next: string[]) => {
    ...
    setTagError(null);
    try {
      const res = await fetch(`/api/notes/${note.id}/tags`, {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
  ...
  const saveConcepts = async (next: string[]) => {
    ...
    try {
      const res = await fetch(`/api/notes/${note.id}/concepts`, {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
      ...
      if (!res.ok) {
        ...
        setConceptError(t('note.saveFailed'));
      }
    } catch (err) {
      setConcepts(concepts);
      setConceptError(t('note.saveFailed'));
    ...
    <div className="rounded-xl border border-line bg-paper p-4">
      ...
      <div className="mb-1 flex items-baseline justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {(concepts.length > 0 || conceptError) && (
            <div>
              ...
              {conceptError && <p className="mt-1 text-[10px] text-red-600">{conceptError}</p>}
            </div>
          )}
          {tagError && <p className="text-[10px] text-red-600">{tagError}</p>}
          <button
      ...
      <div>
        ...
        {tagError && <p className="mt-1 text-[10px] text-red-600">{tagError}</p>}
      </div>
    </div>
  );
}
