...
import type { Note, Concept, NoteAnalysis } from './types';
import type { Lang } from './i18n';
import { readPromptOverrides } from './storage';
import { getLang } from './lang';

// ────────────────────────────────────────────────────────────
// PROMPT 8: DETECT_NOTE_LINKS — 判断新 note 是否是旧 note 的后续/结果
// 模型: gpt-5.4-nano
// ────────────────────────────────────────────────────────────

export const DETECT_NOTE_LINKS_SYSTEM = `你是一个帮用户发现私人笔记之间时间关系的助手。用户写了一条新 note，你要判断它是否是某条旧 note 的后续、结果、验证或反证。

关系类型：
- follow_up：新 note 明显延续旧 note 的话题或行动。
- outcome：新 note 记录了旧 note 中计划、建议、预期、问题的结果。
- validated：新 note 说明旧 note 里的改进、判断或预期有效。
- contradicts：新 note 说明旧 note 的预期不成立，或结果相反。

规则：
1. 只在两条 note 之间存在明确时间链条时返回链接，不要因为同主题/同人物就链接。
2. 如果旧 note 是建议、计划、预期、问题，新 note 是结果或反馈，要优先识别。
3. reason 要用一句自然中文说明“旧 note 说了什么，新 note 证明/回应了什么”。
4. confidence < 0.7 的不要返回。
5. 最多返回 3 条。

返回 JSON：
{
  "links": [
    { "from_note_id": "旧 note id", "type": "validated", "reason": "为什么", "confidence": 0.0-1.0 }
  ]
}`;

export function detectNoteLinksUser(
  newNote: Note,
  newAnalysis: NoteAnalysis | null,
  candidates: Array<{ note: Note; analysis: NoteAnalysis | null }>
): string {
  const candidateText = candidates
    .map(({ note, analysis }, i) => {
      const analysisText = analysis
        ? `\n理解：subject=${analysis.subject}; intent=${analysis.intent}; summary=${analysis.event_summary}; evidence=${analysis.evidence.join(' / ') || '无'}`
        : '';
      return `[${i + 1}] id=${note.id} ${formatDate(note.created_at)}\n${note.content}${analysisText}`;
    })
    .join('\n\n');
  const newAnalysisText = newAnalysis
    ? `subject=${newAnalysis.subject}; intent=${newAnalysis.intent}; summary=${newAnalysis.event_summary}; evidence=${newAnalysis.evidence.join(' / ') || '无'}`
    : '(无)';

  return `新 note：
id=${newNote.id} ${formatDate(newNote.created_at)}
${newNote.content}
...
判断新 note 是否回应、验证、反驳或记录了某条旧 note 的结果，返回 JSON。`;
}

// ────────────────────────────────────────────────────────────
// PROMPT 0: ANALYZE_NOTE — 保存后先生成结构化理解
// 模型: gpt-5.4-nano
// ────────────────────────────────────────────────────────────

export const ANALYZE_NOTE_SYSTEM = `你是一个帮用户理解私人笔记的助手。每条 note 先不要急着分类，先判断它在说什么、主体是谁、对象是谁、证据是什么。

规则：
1. subject 是这条 note 的主要主体，可以是 "user"、某个人名、"team"、"family" 或简短中文描述。
2. object_people 是 note 中被提到、且作为对象/相关人物出现的人名。只放原文出现的人名。
3. intent 描述用户写这条 note 的意图，例如 "自我反思"、"给别人建议"、"记录事实"、"表达情绪"、"计划行动"。
4. candidate_concepts 是可能主题，最多 3 个。必须给 confidence 和 reason。不要因为关键词相似就硬归类，先看主体和语义。
5. 如果用户的洞察目标存在，优先考虑这些方向，但不要强行匹配。
6. evidence 只摘取原文中能支撑判断的短句，最多 3 条。
...
返回 JSON，格式：
{
  ...
  "intent": "写作意图",
  "candidate_concepts": [
    { "concept_title": "主题名", "confidence": 0.0-1.0, "is_new": false, "reason": "为什么" }
  ],
  "evidence": ["原文短句"],
  "confidence": 0.0-1.0
}`;

export function analyzeNoteUser(
  noteContent: string,
  ...
  const conceptList = existingConcepts.length
    ? existingConcepts
        .map((c) => `- "${c.title}"${c.synthesis ? ` · ${c.synthesis.slice(0, 100)}` : ''}`)
        .join('\n')
    : '(还没有主题)';
  const purposeText = purpose.trim() || '(用户还没有设置洞察目标)';
...
5. 先判断 note 的主体/对象是谁。只有当 note 主要描述用户自己的状态、习惯、要求或反思时，才归入"自我要求"、"工作焦虑"这类自我主题。
6. 如果 note 是用户给别人（如孩子、同事、伴侣）的建议、要求或观察，不要仅因为出现"计划、复盘、完成、焦虑"等词就归入用户自己的主题；应优先匹配亲子、教育、沟通、协作等对象相关主题，或新建具体主题，或返回空数组。
...
export function classifyFromAnalysisUser(
  note: Note,
  ...
  const conceptList = existingConcepts.length
    ? existingConcepts
        .map((c) => `- "${c.title}"${c.synthesis ? ` · ${c.synthesis.slice(0, 80)}` : ''}`)
        .join('\n')
    : '(用户还没有任何概念，全都是新的)';
  const candidates = analysis.candidate_concepts
    .map((c) => `- ${c.concept_title} (${c.confidence}): ${c.reason}`)
    .join('\n') || '(无)';
...
- object_people: ${analysis.object_people.join('、') || '无'}
- intent: ${analysis.intent}
- event_summary: ${analysis.event_summary}
- emotion: ${analysis.emotion || '无'}
- evidence: ${analysis.evidence.join(' / ') || '无'}
- candidate_concepts:
${candidates}
...
export function compileUser(
  ...
  const notesText = notes
    .map((n, i) => {
      ...
      const analysis = analyses.find((a) => a.note_id === n.id);
      const analysisText = analysis
        ? `\n结构化理解：subject=${analysis.subject}; intent=${analysis.intent}; summary=${analysis.event_summary}; evidence=${analysis.evidence.join(' / ') || '无'}`
        : '';
      return `[${i + 1}]${isNew ? ' [新]' : ''} ${formatDate(n.created_at)}\n${n.content}${analysisText}`;
    })
    .join('\n\n');
...
export function globalAskUser(question: string, notes: Note[], today: string, purpose = ''): string {
  const notesText = notes
    .map((n, i) => `[${i + 1}] ${formatDate(n.created_at)}\n${n.content}`)
    .join('\n\n');
...
export const PROMPT_LABELS: Record<PromptKey, string> = {
  ANALYZE_NOTE_SYSTEM: '笔记理解',
  DETECT_NOTE_LINKS_SYSTEM: '后续关系',
  CLASSIFY_SYSTEM: '分类',
  COMPILE_SYSTEM: '归纳',
  ASK_SYSTEM: '问答',
...
5. First identify the main subject/object of the note. Only match self-oriented topics such as "Self Expectations" or "Work Anxiety" when the note mainly describes the user's own state, habits, expectations, or reflection.
6. If the note is advice, expectations, or observations about someone else (child, coworker, partner, etc.), do not match the user's self-oriented topics just because words like planning, review, completion, or anxiety appear. Prefer relationship, parenting, education, communication, or collaboration topics; create a specific topic if warranted; otherwise return an empty array.
...
export const ANALYZE_NOTE_SYSTEM_EN = `You are an assistant helping users understand private notes. Before categorizing a note, identify what it says, who the subject is, who it is about, and what evidence supports the interpretation.

Rules:
1. subject is the main subject of the note: "user", a person name, "team", "family", or a short description.
2. object_people are person names explicitly mentioned in the note as objects or related people.
3. intent describes why the user wrote the note, such as "self reflection", "advice to someone else", "fact log", "emotion", or "plan".
4. candidate_concepts contains at most 3 possible topics with confidence and reason. Do not match only by keywords; consider subject and semantics.
5. If a user purpose exists, prefer that direction, but do not force a match.
6. evidence should be short phrases from the note, at most 3.

Return JSON:
{
  "subject": "user or other subject",
  "object_people": ["name"],
  "event_summary": "one sentence",
  "emotion": "emotion or null",
  "intent": "writing intent",
  "candidate_concepts": [
    { "concept_title": "topic", "confidence": 0.0-1.0, "is_new": false, "reason": "why" }
  ],
  "evidence": ["short quote"],
  "confidence": 0.0-1.0
}`;

export const DETECT_NOTE_LINKS_SYSTEM_EN = `You help users find temporal relationships between private notes. Given a new note, decide whether it is a follow-up, outcome, validation, or contradiction of an older note.

Relation types:
- follow_up: the new note clearly continues an older topic or action.
- outcome: the new note records the result of a plan, suggestion, expectation, or problem in an older note.
- validated: the new note shows that an improvement, judgment, or expectation from the old note worked.
- contradicts: the new note shows the old expectation did not hold or the result was opposite.

Rules:
1. Return a link only when there is a clear time chain, not merely the same topic/person.
2. Prefer links where an old note contains a suggestion/plan/expectation/problem and the new note contains a result or feedback.
3. reason should state what the old note said and what the new note proved/responded to.
4. Do not return confidence < 0.7.
5. Return at most 3 links.

Return JSON:
{
  "links": [
    { "from_note_id": "old note id", "type": "validated", "reason": "why", "confidence": 0.0-1.0 }
  ]
}`;
...
const PROMPT_BILINGUAL: Record<PromptKey, Record<Lang, string>> = {
  ANALYZE_NOTE_SYSTEM:  { zh: ANALYZE_NOTE_SYSTEM,  en: ANALYZE_NOTE_SYSTEM_EN },
  DETECT_NOTE_LINKS_SYSTEM: { zh: DETECT_NOTE_LINKS_SYSTEM, en: DETECT_NOTE_LINKS_SYSTEM_EN },
